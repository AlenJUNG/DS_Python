...
# ### iloc[]는 위치 기반 인덱싱만 허용하기 때문에 행과 열 값으로 int or int형의 슬라이싱, 팬시 리스트 값을 입력해야함
# > 불린 인덱싱은 조건을 기술하므로 제약 받지 않음
...
# - 행 위치 : DataFrame index
# - 열 위치 : 컬럼명 입력
# > 단, index가 숫자형일 수 있기 때문에 무조건 문자열을 입력하면 안됌
